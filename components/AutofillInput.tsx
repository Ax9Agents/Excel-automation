...
export default function AutofillInput({
  ...
  useEffect(() => {
    // Always fetch suggestions when focused, regardless of value
    if (showSuggestions) {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        ...
        if (onHoverPreview) onHoverPreview(null); // Clear preview
      }
    };
              ...
              <div className="flex-1">
                ...
                {hoveredIndex === index && suggestion.relatedData && (
                  <p className="text-xs text-blue-600 mt-1 flex items-center gap-1">
                    ...
                    Hovering - Related fields previewed
                  </p>
                )}
              </div>
